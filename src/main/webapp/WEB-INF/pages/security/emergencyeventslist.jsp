...
<title>应急事件查询</title>
<meta name="description" content="应急事件查询">
...
<body>
	...
	<div class="admin-biaogelist" style="position: static;">
		...
		<div class="listbiaoti am-cf">
			<ul><img style="margin:-5px 5px 0px 0px;" src="<c:url value='/img/index/menu8.png'/>">应急事件查询
			</ul>
		...
		<form action="<c:url value='/security/emergencyeventslist.action'/>" method="post">
			<input type="hidden" name="page" value="" />
			<div class="am-btn-toolbars am-btn-toolbar am-kg am-cf">
				<ul>
					<li>
						<input class="gjz-quq" name="name" value="${name }" class="am-form-field am-input-sm am-input-xm" placeholder="记录人姓名" />
	           	 	</li>
					<li>
...
<!-- 						<button type="submit" class="am-btn am-radius am-btn-xs am-btn-success find-quq"><span class="find-quq find">搜索</span></button> -->
		...
		<div class="am-form am-g">
			<table class="am-table am-table-bordered am-table-radius am-table-striped">
				<thead>
					<tr class="am-success">
						...
						<th width="10%" style="text-align:center;">记录人姓名</th>
						<th width="10%" style="text-align:center;">事件类型</th>
						<th width="10%" style="text-align:center;">事件时间</th>
				...
				<tbody id="apply-table">
					...
					<c:forEach items="${paging.list}" var="security" varStatus="status">
						<tr id="1" style="background-color: white;">
							<td align="center" class="oder"><input type="checkbox" id="mychk" class="mychk" name="securityid" value="${security.id}" /></td>
							<td align="center" id="td1" class="oder"><span class="span1" id="num">${ status.index + 1}</span></td>
							<td align="center" class="am-hide-sm-only">${security.name}</td>
							<td align="center" class="am-hide-sm-only">${security.type}</td>
							<td align="center"><fmt:formatDate value="${security.time}" pattern="yyyy-MM-dd HH:mm:ss" /></td>
							<td align="center" class="am-hide-sm-only">${security.note}</td>
							<td style="text-align:center;">
			                	<div class="am-btn-toolbar">
			                    <div class="am-btn-group am-btn-group-xs">
			                    <a type="button" class="am-btn am-btn-default am-btn-xs am-text-secondary am-round" href="<c:url value='/security/toEmergencyeventsModify.action?id=${security.id }'/>" title="修改"><span class="am-icon-pencil-square-o"></span></a> 
			                    <a type="button" class="am-btn am-btn-default am-btn-xs am-text-danger am-round" onclick="javascript:return confirm('您确认要删除吗?');" href="<c:url value='/security/delete.action?id=${security.id }'/>" title="删除"><span class="am-icon-pencil-square-o"></span></a> 
			                    </div>
			                  </div>
                	</td>	
						</tr>
			...
			<div class="am-btn-group am-btn-group-xs">
            	...
            	<a type="button" style="height: 35px;" class="am-btn am-btn-default button-add" href="<c:url value='/security/emergencyevents.action'/>"> </a>
			</div>
...
if(msg!=""){
    ...
    var deleteapplyinfo=function(id){
    	if(confirm("您确认要删除此记录？")){
    		$.ajax({
    			url:"${pageContext.request.contextPath}/security/delete.action?id="+id,
    			type:"POST",
    ...
    var deletechecked=function(){
    	var ids="";
    	alert("sdas");
    	$("input[name='securityid']").each(function(){
    	...
    	if(confirm("您确认要删除所有选中项？")){
    		$.ajax({
    			url:"${pageContext.request.contextPath}/security/deletelist.action?ids="+ids,
    			type:"POST",
    			success:function(result){
    				if(result=="success"){
    					alert("删除成功！");
    ...
    if(type == "应急事件"){
        $("#membertype option[value='应急事件']").attr("selected","selected"); 
    }else if(type == "突发事件"){ 
        $("#type option[value='突发事件']").attr("selected","selected"); 
    }else if(membertype == "其它事件"){
    ...
    var sendUpdate=function(id){
    	window.location.href="${pageContext.request.contextPath}/security/modify.action?id="+id;
    };
