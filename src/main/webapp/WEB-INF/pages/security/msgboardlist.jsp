...
<title>留言板查询</title>
<meta name="description" content="留言板查询">
...
<body>
	...
	<div class="admin-biaogelist" style="position: static;">
		...
		<div class="listbiaoti am-cf">
			<ul><img style="margin:-5px 5px 0px 0px;" src="<c:url value='/img/index/menu8.png'/>">留言板查询
			</ul>
		...
		<form action="<c:url value='/msgboard/msgboardlist.action'/>" method="post">
			<input type="hidden" name="page" value="" />
			<div class="am-btn-toolbars am-btn-toolbar am-kg am-cf">
				<ul>
					<li>
						<input class="gjz-quq" name="msgboardname" value="${msgboardname }" class="am-form-field am-input-sm am-input-xm" placeholder="姓名" />
	           	 	</li>
					<li>
...
<!-- 						<button type="submit" class="am-btn am-radius am-btn-xs am-btn-success find-quq"><span class="find-quq find">搜索</span></button> -->
		...
		<div class="am-form am-g">
			<table class="am-table am-table-bordered am-table-radius am-table-striped">
				<thead>
					<tr class="am-success">
						...
						<th width="30%" style="text-align:center;">留言内容</th>
						<th width="8%" style="text-align:center;">操作</th>
					</tr>
				</thead>
				<tbody id="apply-table">

					<c:forEach items="${paging.list}" var="msgboard" varStatus="status">
						<tr id="1" style="background-color: white;">
							<td align="center" class="oder"><input type="checkbox" id="mychk" class="mychk" name="msgboardid" value="${msgboard.msgboardid}" /></td>
							<td align="center" id="td1" class="oder"><span class="span1" id="num">${ status.index + 1}</span></td>
							<td align="center" class="am-hide-sm-only">${msgboard.msgboardname}</td>
							<td align="center"><fmt:formatDate value="${msgboard.msgboardtime}" pattern="yyyy-MM-dd HH:mm:ss" /></td>
							<td align="center" class="am-hide-sm-only">${msgboard.msgboardphone}</td>
							<td align="center" class="am-hide-sm-only">${msgboard.ex1}</td>
							<td style="text-align:center;">
			                	<div class="am-btn-toolbar">
			                    <div class="am-btn-group am-btn-group-xs">
			                    <a type="button" class="am-btn am-btn-default am-btn-xs am-text-secondary am-round" href="<c:url value='/msgboard/toMsgboardModify.action?id=${msgboard.msgboardid }'/>" title="修改"><span class="am-icon-pencil-square-o"></span></a> 
			                    <a type="button" class="am-btn am-btn-default am-btn-xs am-text-danger am-round" onclick="javascript:return confirm('您确认要删除吗?');" href="<c:url value='/msgboard/delete.action?id=${msgboard.msgboardid }'/>" title="删除"><span class="am-icon-pencil-square-o"></span></a> 
			                    </div>
			                  </div>
                	</td>	
						</tr>
			...
			<div class="am-btn-group am-btn-group-xs">
            	...
            	<a type="button" style="height: 35px;" class="am-btn am-btn-default button-add" href="<c:url value='/msgboard/msgboard.action'/>"> </a>
			</div>
...
if(msg!=""){
    ...
    var deleteapplyinfo=function(id){
    	if(confirm("您确认要删除此记录？")){
    		$.ajax({
    			url:"${pageContext.request.contextPath}/msgboard/delete.action?id="+id,
    			type:"POST",
    ...
    var deletechecked=function(){
    	var ids="";
    	$("input[name='msgboardid']").each(function(){
    	...
    	if(confirm("您确认要删除所有选中项？")){
    		$.ajax({
    			url:"${pageContext.request.contextPath}/msgboard/deletelist.action?ids="+ids,
    			type:"POST",
    			success:function(result){
    				if(result=="success"){
    					alert("删除成功！");
    ...
    var sendUpdate=function(id){
    	window.location.href="${pageContext.request.contextPath}/msgboard/modify.action?id="+id;
    };
